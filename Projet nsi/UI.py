...
def menu(screen):
    ...
    pygame.draw.rect(
        screen, parametres.BLEU_FONCE, bouton.parametre_bt, border_radius=4
    )
    fonction.afficher_texte(
...
def play(screen, grille_jeu, joueur):
    ...
    for c in grille_jeu.values():
        if c.contenu != None:
            pygame.draw.circle(
                ...
                c.contenu.couleur,
                (
                    (screen.get_width() - parametres.taille_grille * taille_cell) // 2
                    + c.x * taille_cell
                    + taille_cell // 2,
                    (screen.get_height() - parametres.taille_grille * taille_cell) // 2
                    + c.y * taille_cell
                    + taille_cell // 2,
                ),
                taille_cell // 2 - 2,
    ...
    fonction.draw_health_bar(
        ...
        xbouton + 550,
        ybouton - 400,
        current=entities.j1.xp,
        maximum=10,  # ← toujours 10 xp par niveau
        label=f"Niv.{entities.j1.niveau} : ",
        color_override=(250, 204, 21),  # ← jaune/or
    )
...
def parametre(screen):
    ...
    pygame.draw.rect(
        screen,
        (
            parametres.BLEU_FONCE
            if not parametres.facile_clicked
            else parametres.BLEU_CLAIR
        ),
        bouton.facile_bt,
    )
    fonction.afficher_texte(
    ...
    pygame.draw.rect(
        ...
        (
            parametres.BLEU_FONCE
            if not parametres.moyen_clicked
            else parametres.BLEU_CLAIR
        ),
        bouton.moyen_bt,
    )
    ...
    pygame.draw.rect(
        ...
        (
            parametres.BLEU_FONCE
            if not parametres.difficile_clicked
            else parametres.BLEU_CLAIR
        ),
        bouton.difficile_bt,
    )
